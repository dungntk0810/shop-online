 ...
 * @author mylov
 */
@WebServlet(name = "OrderDetails", urlPatterns = {"/orderd"})
public class OrderDetails extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<OrderDetail> list = tid.getAllOrderInfoByUid(a.getId());
        List<Statistical> listD = new ArrayList<>();
        Date date = null;

        for (OrderDetail o : list) {
            Date date1 = o.getDate();
            if(!date1.equals(date)){
            listD.add(tid.gePriceOrderInfoByDate(o.getDate()));
            date = date1;
            }
        }
